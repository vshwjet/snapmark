...
export function getElementInfo(el: Element): ElementInfo {
  ...
  // Text content — only for leaf-ish elements with short text
  let text: string | undefined
  if (TEXT_TAGS.has(tag)) {
    const raw = el.textContent?.replace(/\s+/g, ' ').trim() ?? ''
    if (raw.length > 0 && raw.length <= 80) text = raw
  }
  // img alt, input value/placeholder as text fallback
  if (!text) {
  ...
  // Accessible label (aria-label > title > placeholder)
  const label =
  ...
  // Meaningful attributes for developers
...
/** Class-based path — useful for Tailwind class names. E.g. `.flex.gap-2 > button.btn-primary` */
function buildClassPath(el: Element): string {
  ...
  for (let depth = 0; depth < 5 && current && current !== document.body; depth++) {
    ...
    if (classes.length > 0) {
      // Take up to 3 most specific classes
...
/** Tag-based path — reliable fallback. E.g. `nav > ul > li > a` */
